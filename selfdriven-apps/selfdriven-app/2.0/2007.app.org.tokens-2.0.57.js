/*
	{
    	title: "Org; Tokens", 	
    	design: "https://slfdrvn.io/tokens"
  	}
*/
...
app.add(
{
	...
	code: function (param, response)
	{
		app.invoke('util-dashboard',
		{
			...
			[
				{
					name: 'org-tokens-community-dashboard-total-earned',
					containerSelector: '#org-tokens-community-dashboard-total-earned',
					template: '{{total}}',
					formatController: 'org-tokens-format',
					defaults:
					{
						total: 0
					},
				...
				{
					name: 'org-tokens-community-dashboard-total-used',
					containerSelector: '#org-tokens-community-dashboard-total-used',
					template: '{{total}}',
					formatController: 'org-tokens-format',
					defaults:
					{
		...
		app.invoke('org-tokens-community-dashboard-transactions');
	}
});

app.add(
{
	name: 'org-tokens-format',
...
app.add(
{
	name: 'org-tokens-community-dashboard-transactions',
	code: function (param, response)
	{
		var data = app.get(
		{
			scope: 'org-tasks-dashboard',
			valueDefault: {}
		});
		...
		{
			object: 'action',
			container: 'org-tokens-community-dashboard-transactions-view',
			context: 'org-tokens-community-dashboard-transactions',
			filters: filters,
			options:
			{
			...
			format:
			{
				...
				row:
				{
					data: 'data-id="{{id}}"',
					class: 'd-flex',
					controller: 'org-tokens-community-dashboard-transactions-format'
				},
				...
				[
					{
						...
						data: 'data-controller="org-token-summary" data-context="{{guid}}" data-id="{{guid}}"'
					},
					{
						caption: 'Type',
						name: 'typeinfo',
...
app.add(
{
	name: 'org-tokens-community-dashboard-transactions-format',
	code: function (row)
	{
...
app.add(
{
	...
	code: function (param, response)
	{
		...
		else
		{
			...
			app.show('#org-tokens-use-dashboard-on-chain', availableTokens);
			app.invoke('org-tokens-use-dashboard-spend');
			app.invoke('org-tokens-use-dashboard-vote');
		}
	}
});
...
app.add(
{
	name: 'org-tokens-use-dashboard-spend',
	code: function (param, response)
	{
		if (response == undefined)
		{
			mydigitalstructure.cloud.search(
			{
				...
				callback: 'org-tokens-use-dashboard-spend',
				callbackParam: param
			});
		}
		else
		{
			...
			app.show('#org-tokens-use-dashboard-spend', movableTokensTotal);
			app.show('#org-tokens-use-dashboard-spend-spendable', spendableTokensTotal);
		}
	}
});
...
app.add(
{
	name: 'org-tokens-use-dashboard-vote',
	code: function (param, response)
	{
		if (response == undefined)
		{
			mydigitalstructure.cloud.search(
			{
				...
				callback: 'org-tokens-use-dashboard-vote',
				callbackParam: param
			});
		}
		else
		{
			...
			app.show('#org-tokens-use-dashboard-vote', voteTokensTotal);
		}
	}
});
...
app.add(
{
	name: 'org-tokens-use-on-chain',
	code: function (param, response)
	{
		app.invoke('org-tokens-use-on-chain-profile');
		app.invoke('org-tokens-use-on-chain-move');
	}
});

app.add(
{
	name: 'org-tokens-use-on-chain-profile',
	code: function (param, response)
	{	
		if (response == undefined)
		{
			mydigitalstructure.cloud.search(
			{
				...
				callback: 'org-tokens-use-on-chain-profile'
			});
		}
		else
		{
			var profileView = app.vq.init({queue: 'org-tokens-use-on-chain-profile'});
			...
			else
			{
				...
				app.set(
				{
					scope: 'org-tokens-use-on-chain-profile',
					context: 'address',
					value: data.key
				});
			}
			...
			[
					...
					'<div class="col-6">',
						'<div class="card">',
							...
							'</div>',
							'<div class="card-body text-center" id="org-tokens-use-on-chain-profile-community">',
			]);
			...
			profileView.render('#org-tokens-use-on-chain-profile');

			app.invoke('org-tokens-use-on-chain-profile-community');
		}		
	}	
});
...
app.add(
{
	name: 'org-tokens-use-on-chain-profile-community',
	code: function (param, response)
	{	
		if (response == undefined)
		{
			mydigitalstructure.cloud.search(
			{
				...
				callback: 'org-tokens-use-on-chain-profile-community'
			});
		}
		else
		{
			var profileView = app.vq.init({queue: 'org-tokens-use-on-chain-profile-community'});
			...
			profileView.render('#org-tokens-use-on-chain-profile-community');
		}		
	}	
});
...
app.add(
{
	name: 'org-tokens-use-spend',
	code: function (param, response)
	{
		app.invoke('org-tokens-use-spend-move');
	}
});

app.add(
{
	name: 'org-tokens-use-spend-move',
	code: function (param, response)
	{
		var filters =
		[
		...
		{
			object: 'action',
			container: 'org-tokens-use-spend-move-view',
			context: 'org-tokens-use-spend-move',
			filters: filters,
			options:
			{
				...
				select:
				{
					controller: 'org-tokens-use-spend-move-process',
					caption: 'Move',
					selected: true,
					data: 'data-id={{id}}'
				}
			...
			format:
			{
				...
				row:
				{
					data: 'data-id="{{id}}"',
					class: '',
					controller: 'org-tokens-use-spend-format'
				},
...
app.add(
{
	name: 'org-tokens-use-spend-format',
	code: function (row)
	{
		row.date = app.invoke('util-view-date-clean', row.date)
...
app.add(
{
	name: 'org-tokens-use-spend-move-init',
	code: function (param, response)
	{
		var inputs = app.set(
		{
			scope: 'org-tokens-use-spend-move',
			context: 'inputs',
			value: $('[data-context="org-tokens-use-spend-move"] input.myds-view-table-select:checked')
		});
		...
		app.invoke('org-tokens-use-spend-move-process');
	}
});

app.add(
{
	name: 'org-tokens-use-spend-move-process',
	code: function (param, response)
	{
		var index = app.get(
		{
		...
		if (index < inputIDs.length)
		{
			...
			mydigitalstructure.cloud.save(
			{
				...
				callback: 'org-tokens-use-spend-move-next'
			});
		}
		else
		{
...
app.add(
{
	name: 'org-tokens-use-spend-move-next',
	code: function (param, response)
	{
		var index = app.get(
		{
...
app.add(
{
	name: 'org-tokens-use-spend-account',
	code: function (param, response)
	{
		var filters =
		[
		...
		{
			object: 'action',
			container: 'org-tokens-use-spend-account-view',
			context: 'org-tokens-use-spend-account',
			filters: filters,
			options:
			{
...
app.add(
{
	name: 'org-tokens-use-on-chain-move',
	code: function (param, response)
	{
		var filters =
		[
		...
		{
			object: 'action',
			container: 'org-tokens-use-on-chain-move-view',
			context: 'org-tokens-use-on-chain-move',
			filters: filters,
			options:
			{
				noDataText: '<div class="p-4">No tokens available for moving.</div>',
				...
				select:
				{
					controller: 'org-tokens-use-on-chain-move-process',
					caption: 'Move',
					selected: true,
					data: 'data-id={{id}}'
				},
			...
			format:
			{
				...
				row:
				{
					data: 'data-id="{{id}}"',
					class: '',
					controller: 'org-tokens-use-on-chain-move-format'
				},
...
app.add(
{
	name: 'org-tokens-use-on-chain-move-format',
	code: function (row)
	{
		row.date = app.invoke('util-view-date-clean', row.date)
...
app.add(
{
	name: 'org-tokens-use-on-chain-query',
	code: function (param, response)
	{
		if (param.status == 'shown')
		{
			...
			app.show('#org-tokens-use-on-chain-query-view',
			[
				'<div class="card mt-4">',
					'<div class="card-body">',
			...
			{
				scope: 'org-tokens-use-on-chain-query-response',
			...
			if (refreshOnChainData)
			{
				...
				if (_.isSet(address))
				{
					...
					mydigitalstructure.cloud.invoke(
					{
						...
						callback: 'org-tokens-use-on-chain-query-response'
					});
				}
				else
				{
					...
					[
						'<div class="card mt-4">',
							'<div class="card-body">',
								'<div class="text-center">',
									'<div class="text-secondary mb-2">You need to set up your <a href="#org-me-on-chain-edit">on-chain profile</a></div>',
								'</div>',
							'</div>',
						'</div>'
					]);
			...
			{
				app.invoke('org-tokens-use-on-chain-query-show', param, onChainData)
			}	
		}
	}
});
...
app.add(
{
	name: 'org-tokens-use-on-chain-query-response',
	code: function (param, response)
	{
...
app.add(
{
	name: 'org-tokens-use-on-chain-query-show',
	code: function (param, onChainData)
	{
		var onChainQuery = app.vq.init({queue: 'org-tokens-use-on-chain-query-view'});
		...
		onChainQuery.render('#org-tokens-use-on-chain-query-view')
	}
});

app.add(
{
	name: 'org-tokens-use-vote',
	code: function (param, response)
	{
		if (response == undefined)
		{
		...
		else
		{
			...
			app.set(
			{
				scope: 'org-tokens-use-dashboard-use-vote',
				field: 'total',
				value: voteTokensTotal
			});

			app.show('#org-tokens-use-dashboard-use-vote', voteTokensTotal);

			app.invoke('org-tokens-use-vote-community')
		}
	}
});
...
app.add(
{
	name: 'org-tokens-use-vote-community',
	code: function (param, response)
	{
		if (response == undefined)
		{
			mydigitalstructure.cloud.search(
			{
				...
				callback: 'org-tokens-use-vote-community',
				callbackParam: param
			});
		}
		else
		{
			...
			app.show('#org-tokens-use-dashboard-use-vote-community', voteTokensCommunityTotal);
			...
			app.show('#org-tokens-use-dashboard-use-vote-stake-allocation', allocation);
		}
	}
});

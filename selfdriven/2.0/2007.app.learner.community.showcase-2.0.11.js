/*
	{
    	title: "Learner; Community; Showcase", 	
...
app.add(
{
	name: 'learner-community-showcase',
	code: function (param, response)
	{
		...
		*/
		...
		app.invoke('learner-community-showcase-dashboard');
	}
});

app.add(
{
	name: 'learner-community-showcase-dashboard',
	code: function (param, response)
	{
		var data = app.get(
		{
			scope: 'learner-community-showcase-dashboard',
			valueDefault: {}
		});
		...
		if (response == undefined)
		{
			...
			mydigitalstructure.cloud.search(
			{
				...
				callback: 'learner-community-showcase-dashboard'
			})
		}
		else
		{
			var showcaseView = app.vq.init({queue: 'learner-community-showcase-dashboard'});
			...
			if (response.data.rows.length == 0)
			{
				...
				[
					'<div class="text-muted text-center">',
						'There are no community showcase projects that match this search.',
					'</div>'
				]);
			}
			else 
			{
					...
					'<div class="col-sm-12 mb-3 mt-0">',
						'<div class="card">',
							...
							'<div class="collapse myds-collapse" id="learner-community-skills-dashboard-collapse-{{id}}"',
								...
								'data-project="{{id}}">',
								'<div class="card-body py-2 px-4" id="learner-community-skills-dashboard-view-{{id}}">',
									'{{info}}',
								'</div>',
							'</div>',
						'</div>',
				...
				_.each(response.data.rows, function (row)
				{
					...
					if (row.shopstatus == 1)
					{
						row.info += '<button class="mb-4 mt-0 mr-2 btn btn-default btn-outline btn-sm myds-navigate" id="learner-community-showcase-dashboard-view-project-story-{{id}}" data-context="' + row.barcode + '" data-id="' + row.barcode + '" data-controller="util-community-shop">Buy</button>';
					}
			...
			showcaseView.render('#learner-community-showcase-view');
		}
	}
});
...
app.add(
{
	name: 'x-learner-community-showcase-dashboard-show',
	code: function (param, response)
	{
...
app.add(
{
	name: 'learner-community-showcase-summary',
	code: function (param, response)
	{	
		var id = app.get(
		{

...
[	
	{
		...
		code: function (param)
		{
			...
			if (_.isArray(data.teams))
			{
				...
				contentColumn1.push(
				{
					ul: _.map(data.teams, function (team)
									{
										return {stack: [{text: team['projectteam.project.description'], margin: [ 0, 0, 0, 4]}]}
									}),
					margin: [ 8, 4, 0, 0 ],
				});
			}

			if (_.isArray(data.skills))
			{
				contentColumn1.push(
				{
				...
				contentColumn1.push(
				{
					ul: _.map(data.skills, function (skill)
									{
										return {stack: [skill.attributetext, {text: skill.notes, color: '#888888', fontSize: 10, margin: [0, 0, 0, 4]}]}
									}),
					margin: [8, 4, 0, 0]
				});
			}

			if (false && _.isArray(data.learningPartners))
			{
				contentColumn1.push(
				{
						text: 'Learning Partners',
						margin: [ 0, 14, 0, 0 ],
						fontSize: 14,
						bold: false,
						color: '#AFB42B'
				...
				contentColumn1.push(
				{
					ul: _.map(data.learningPartners, function (learningPartner)
					{
						return {
							stack: 	[
											learningPartner['relationship.othercontactperson.firstname'] + ' ' +
											learningPartner['relationship.othercontactperson.surname'],
											{
												text: learningPartner['typetext'],
												color: '#888888',
												fontSize: 10
											},
											{
												text: learningPartner['relationship.othercontactperson.email'],
												color: '#888888',
												fontSize: 10,
												margin: [0, 0, 0, 10]
											}
			...
			if (_.isArray(data.elements))
			{
				...
				{
					element.x2 = (numeral(element.value).value() / 100 * 260) + 4;
			...
			[
				{
					text: whoamiUser.contactbusinesstext,
					margin: [ 0, 0, 0, 0 ],
					fontSize: 18,
					bold: true,
					color: '#2f4050',
				...
				},
				{
					margin: [ 0, 10, 0, 6 ],
					alignment: 'center',
					qr: 'https://www.selfdriven.cloud/verify/' + whoamiUser.guid,
					fit: '100',
					background: '#F1F8E9'
				},
				{
			...
			if (_.isArray(data.learningPartners))
			{
				contentColumn2.push(
				{
						text: 'Learning Partners',
				...
				_.each(data.learningPartners, function (learningPartner)
				{
					contentColumn2 = _.concat(contentColumn2,
					{
						text: learningPartner['relationship.othercontactperson.firstname'] + ' ' +
								learningPartner['relationship.othercontactperson.surname'],
						alignment: 'center',
						margin: [0, 10, 0, 0]
					},
					...
					if (learningPartner['notes'] != '')
					{
						contentColumn2.push(
						{
							text: learningPartner['notes'],
							color: '#888888',
							fontSize: 10,
							alignment: 'center'
						})
			...
			if (_.isArray(data.endorsements))
			{
				contentPage2Column1.push(
				{
						text: 'Endorsements',
						margin: [ 0, 24, 0, 0 ],
						fontSize: 12,
				...
				contentPage2Column1.push(
				{
					ul: _.map(data.endorsements, function (endorsement)
					{
						return {
							stack: 	[
										{
											text: endorsement['subject'],
											bold: true
										},
										endorsement['description'],
										{
											text: endorsement['action.createduser.contactpersontext'] + ', ' +
														app.invoke('util-date', {date: endorsement.createddate, format: 'DD MMM YYYY'}),
											color: '#888888',
											margin: [0, 0, 0, 10],
											fontSize: 10
										}
					...
					table:
					{
						...
						[
							[
								{
									margin: [ 20, 14, 30, 20 ],
									stack: contentColumn1
								},
								{
									fillColor: '#F1F8E9',
									margin: [ 10, 20, 20, 20 ],
									stack: contentColumn2
								}
							]
						]
					}
					...
					table:
					{
						...
						[
							[
								{
									margin: [ 20, 14, 30, 20 ],
									stack: contentPage2Column1
								},
								{
									fillColor: '#F1F8E9',
									margin: [ 10, 20, 20, 20 ],
									stack: contentPage2Column2
								}
							]
						]
					}
			...
			{		
				...
				pageMargins: [ 20, 30, 36, 40 ],
				info:
				{
					title: 'selfdriven Learning Profile',
					author: whoami.thisInstanceOfMe.user.firstname + ' ' + whoami.thisInstanceOfMe.user.surname,
					subject: 'selfdriven Learning Profile',
					keywords: 'selfdriven Learning Profile',
				},
				permissions:
				{
									...
									[
										{
											...
											[
												'[ ',
												whoami.buildingMe.about.name,
												' | ',
												moment().format('DD MMM YYYY h:mm a'),
												(audit.reference!=undefined?' | ' + audit.reference:''),
												(audit.guid!=undefined?' | ' + audit.guid:''),
												' ]'
											],
											fontSize: 8,
											color: '#676a6c',
										...
										},
										{
											...
											[
												{
													text: currentPage.toString(), fontSize: 16
												},
												' / ',
												pageCount
											],
				...
				images:
				{
					profile: whoamiProfile._profileimage
				},
			...
			param = app._util.param.set(param, 'definition', definition);
			param = app._util.param.set(param, 'saveToCloudStorage', false);
			param = app._util.param.set(param, 'saveLocal', true);
			param = app._util.param.set(param, 'filename', 'selfdriven-learning-profile.pdf');

			app.invoke('util-pdf-make', param);
		}
	},

	{
		name: 'util-pdf-project-story',
		code: function (param)
		{
			var subject = app._util.param.get(param, 'subject').value;
			var tableHeader = app._util.param.get(param, 'header').value;
			...
			if (_.isArray(data.tasks))
			{
				...
				contentColumn1.push(
				{
					ul: _.map(data.tasks, function (task)
									{
										return {stack: [{text: task['title'], margin: [ 0, 0, 0, 4]}]}
									}),
					margin: [ 8, 4, 0, 0 ],
				});
			}

			if (!_.isEmpty(data.accountability))
			{
				contentColumn1.push(
				{
						text: 'Fitness (Review Feedback)',
						margin: [0, 14, 0, 0],
						fontSize: 14,
						bold: false,
						color: '#C8B69A'
				...
				contentColumn1.push(
				{
					ul: _.map(data.accountability, function (accountability)
									{
										return {stack: [accountability.subject, {text: accountability.description, color: '#888888', fontSize: 10, margin: [0, 0, 0, 4]}]}
									}),
					margin: [8, 4, 0, 0]
				});
			}

			if (!_.isEmpty(data.reflections))
			{
				contentColumn1.push(
				{
						text: 'Reflections',
				...
				contentColumn1.push(
				{
					ul: _.map(data.reflections, function (reflection)
									{
										return {stack: [reflection.description]}
									}),
					margin: [8, 4, 0, 0]
				});
			}
			...
			[
				...
				},
				{
					margin: [ 0, 10, 0, 6 ],
					alignment: 'center',
					qr: 'https://www.selfdriven.cloud/verify/#project/' + whoamiUser.guid,
					fit: '80',
					background: '#F1F8E9'
				},
				{
			...
			if (_.isArray(data.team))
			{
				...
				_.each(data.team, function (team)
				{
					contentColumn2 = _.concat(contentColumn2,
					{
						text: team['projectteam.contactperson.firstname'] + ' ' +
								team['projectteam.contactperson.surname'],
						alignment: 'center',
						margin: [0, 10, 0, 0]
					},
					{
			...
			{		
				...
				info:
				{
					title: 'Project Story',
					author: whoami.thisInstanceOfMe.user.firstname + ' ' + whoami.thisInstanceOfMe.user.surname,
					subject: 'Project Story',
					keywords: 'Project Story',
				},
				permissions:
				{
   					printing: 'highResolution',
				...
				{
					profile: whoamiProfile._profileimage,
					logo: '/site/2007/selfdriven-logo-with-line.png'
				},
			...
			param = app._util.param.set(param, 'saveToCloudStorage', false);
			param = app._util.param.set(param, 'saveLocal', true);
			param = app._util.param.set(param, 'filename', 'project-story.pdf');

			app.invoke('util-pdf-make', param);
		}
	},
	{
		name: 'util-pdf-profile-level-up',
		code: function (param)
		{
			var subject = app._util.param.get(param, 'subject').value;
			var tableHeader = app._util.param.get(param, 'header').value;
			...
			if (subject == undefined)
			{
				subject = 'My Level Up Profile (CONFIDENTIAL)' + ' | ' + name;
			}
			...
			if (_.has(data, 'skills.gained'))
			{
				if (_.isArray(data.skills.gained))
				{
					if (data.skills.gained.length != 0)
					{
						contentColumn1.push(
						{
								text: 'I Want To Improve (Skills)',
								margin: [ 0, 0, 0, 0 ],
								fontSize: 16,
								bold: true,
								color: '#AFB42B'
						...
						contentColumn1.push(
						{
							ul: _.map(data.skills.gained, function (skill)
							{
								skill._communitySkill = _.find(communitySkills, function (communitySkill)
								{
									return communitySkill.uri == skill.uri
								});

								skill._caption = skill.uri;
								if (_.isSet(skill._communitySkill))
								{
									skill._caption = skill._communitySkill.caption
								}

								return {stack: [skill._caption, {text: skill.notes, color: '#888888', fontSize: 10, margin: [0, 0, 0, 4]}]}
							}),
							margin: [ 8, 4, 0, 0 ],
						});
					}
			...
			if (_.isArray(data.outcomes))
			{
				if (_.isArray(data.outcomes))
				{
					if (data.outcomes.length != 0)
					{
						contentColumn1.push(
						{
							text: 'By Doing Challenges (Outcomes)',
							margin: [0, 14, 0, 0],
							fontSize: 16,
							bold: true,
							color: '#AFB42B'
						...
						contentColumn1.push(
						{
							ol: _.map(data.outcomes, function (outcome)
							{
			...
			if (_.has(data, 'team.members'))
			{
				if (_.isArray(data.team.members))
				{
					if (data.team.members.length != 0)
					{
						...
						contentColumn1.push(
						{
							ul: _.map(data.team.members, function (member)
							{
								return {stack: [member.name]}
							}),
							margin: [8, 4, 0, 0]
						});
					}
			...
			if (_.has(data, 'support.connections'))
			{
				if (_.isArray(data.support.connections))
				{
					if (data.support.connections.length != 0)
					{
						contentColumn1.push(
						{
								text: 'Supported By',
						...
						contentColumn1.push(
						{
							ul: _.map(data.support.connections, function (connection)
							{
								return {stack: [connection.name]}
							}),
							margin: [8, 4, 0, 0]
						});
					}
			...
			if (_.has(data, 'recognition.types'))
			{
				if (_.isArray(data.recognition.types))
				{
					if (data.recognition.types.length != 0)
					{
						contentColumn1.push(
						{
								text: 'For',
						...
						contentColumn1.push(
						{
							ul: _.map(data.recognition.types, function (type)
							{
								type._recognitionType = _.find(recognitionTypes, function (recognitionType)
								{
									return recognitionType.uri == type.uri
								});

								type._caption = type.name;
								if (_.isSet(type._recognitionType))
								{
									type._caption = type._recognitionType.caption
								}

								return {stack: [type._caption]}
							}),
							margin: [8, 4, 0, 0]
						});
					}
...
*/			
			...
			{		
				...
				info:
				{
					title: 'selfdriven Level Up Profile',
					author: whoami.thisInstanceOfMe.user.firstname + ' ' + whoami.thisInstanceOfMe.user.surname,
					subject: 'selfdriven Level Up Profile',
					keywords: 'selfdriven Level Up Profile',
				},
				permissions:
				{
   				printing: 'highResolution',
			...
			param = app._util.param.set(param, 'saveToCloudStorage', false);
			param = app._util.param.set(param, 'saveLocal', true);
			param = app._util.param.set(param, 'filename', 'selfdriven-level-up-profile.pdf');
...
[
	{
		...
		code: function (param)
		{
			var definition = app._util.param.get(param, 'definition').value;
			...
			if (definition.images != undefined)
			{
				var images = [];
				_.each(definition.images, function (imageURI, imageName)
				{
					if (!_.startsWith(imageURI, 'data:'))
					{
						images.push({name: imageName, uri: imageURI});
					}
				});

				app.set(
				{
					scope: 'util-pdf-process-images',
					context: 'images',
					value: images
				});
				...
				app.invoke('util-pdf-make-process-images', param);
			}
			else
			{
				app.invoke('util-pdf-make-process', param);
			}
		}
	},
	{
		name: 'util-pdf-make-process-images',
		code: function (param)
		{
			var definition = app.get(
			{
				scope: 'util-pdf-make',
				context: 'definition'
			});
			...
			if (imageIndex < images.length)
			{
				...
				app.invoke('util-image-to-base64',
				{
					scale: 1,
					imageURI: image.uri, 
					imageType: _.last(_.split(image.uri, '.')),
					onComplete: 'util-pdf-make-process-images',
					set:
					{
						scope: 'util-pdf-process-images',
						context: 'processed',
					...
					autoSetWidthHeight: true
				});
			}
			else
			{
				var processedImages = app.get(
				{
					scope: 'util-pdf-process-images',
					context: 'processed'
				});

				definition.images = _.assign(definition.images, processedImages);
	...
	},	
	{
		...
		{	
			...
			var type = app._util.param.get(param, 'type').value;
			var filename = app._util.param.get(param, 'filename').value;
			var showHide = app._util.param.get(param, 'showHide', {default: false}).value;
			var saveToCloudStorage = app._util.param.get(param, 'saveToCloudStorage', {default: true}).value;
			var saveLocal = app._util.param.get(param, 'saveLocal', {default: false}).value;
			...
			if (typeof window.pdfMake !== 'undefined' && typeof window.pdfMake.addFontContainer !== 'undefined') {
				window.pdfMake.addFontContainer(fontContainer);
			}
			...
			if (saveToCloudStorage)
			{
				pdfMake.createPdf(definition).getBlob().then(function (pdf)
				{
					if (showHide)
	 				{
						$(selector).addClass('hidden').addClass('d-none');
					}

					var formData = new FormData();
					formData.append('file0', pdf);
					...
					$.ajax('/rpc/attach/?method=ATTACH_FILE',
					{
						...
						{
							mydigitalstructure._util.doCallBack(param, data)
						},
						error: function(data)
						{
							app.notify(data)
...

	{
		...
		{	
			...
			{		
				...
				info:
				{
					title: 'HARPS Checklist',
					author: 'User Name',
					subject: 'Audit by / supplier / date',
					keywords: 'HARPS Checklist',
				},
				permissions:
				{
   				printing: 'highResolution',
				...
				footer: function(currentPage, pageCount)
				{ 
					return {text: currentPage.toString() + ' of ' + pageCount, margin: 20, alignment: 'right'};
				},
				content:
				[
					{
					...
					{ 	text: 'Tables support the same width definitions as standard columns:'},
					{
						bold: true,
						ul: [
							'auto',
					...
					{text: 'CHECKLIST', pageBreak: 'before', style: 'header'},
					{
						style: 'tableExample',
						table:
						{
				...
				images:
				{
					logo: '/site/1991/1991.logo-1.0.4.png',
					agc: '/site/1991/agcertifiers.png'
				},
			...
			app.invoke('util-pdf-make', {definition: definition})
		}
	},
}

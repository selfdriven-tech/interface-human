/*
	{
    	title: "Learner; Community; Facilitators", 	
...
app.add(
{
	name: 'learner-community-facilitators',
	code: function (param, response)
	{
		var projects = app.get(
		{
			scope: 'learner-community-facilitators',
			context: 'projects'
		});
		...
		if (response == undefined && projects == undefined)
		{
			mydigitalstructure.cloud.search(
			{
				...
				callback: 'learner-community-facilitators'
			});
		}
		else
		{
			app.vq.init({queue: 'learner-community-facilitators-projects'});
			app.vq.add(
			[
				'',
			...
			{
				type: 'template',
				queue: 'learner-community-facilitators-projects'
			});

			app.vq.add(
			[
				'<button type="button" class="btn btn-primary btn-sm dropdown-toggle" data-toggle="dropdown" id="learner-community-faciliators-projects-filter" aria-expanded="false">',
					'<span class="dropdown-text">All</span>',
				'</button>',
				'<ul class="dropdown-menu mt-1"',
					'data-controller="learner-community-facilitators-dashboard"',
			...
			{
				app.vq.add({useTemplate: true, queue: 'learner-community-facilitators-projects'}, project)
			});
			...
			app.vq.render('#learner-community-facilitators-projects',
			{
				queue: 'learner-community-facilitators-projects'
			});

			app.invoke('learner-community-facilitators-dashboard');
		}
	}
});
...
app.add(
{
	name: 'learner-community-facilitators-dashboard',
	code: function (param, response)
	{
		var data = app.get(
		{
			scope: 'learner-community-facilitators-dashboard',
			valueDefault: {}
		});
		...
		if (response == undefined)
		{
			...
			mydigitalstructure.cloud.search(
			{
				...
				callback: 'learner-community-facilitators-dashboard'
			})
		}
		else
		{
			...
			app.invoke('learner-community-facilitators-dashboard-show')
		}
	}
});
...
app.add(
{
	name: 'learner-community-facilitators-dashboard-show',
	code: function (param, response)
	{
		var data = app.get(
		{
		...
		if (response == undefined)
		{
			...
			mydigitalstructure.cloud.search(
			{
				...
				callback: 'learner-community-facilitators-dashboard-show'
			});
		}
		else
		{
			app.vq.init({queue: 'learner-community-facilitators-dashboard'});
				...
				if (data.project != -1)
				{
					...
					if (!_.isUndefined(project))
					{
						if (project.notes != '')
						{
							...
							{
								queue: 'learner-community-facilitators-dashboard'
							});
						}
					}
				}
			...
			else 
			{
				...
				_.each(response.data.rows, function (row)
				{
					...
					app.vq.add({useTemplate: true, queue: 'learner-community-facilitators-dashboard'}, row)
				});
			...
			app.vq.render('#learner-community-facilitators-view',
			{
				queue: 'learner-community-facilitators-dashboard'
			});
		}
...
app.add(
{
	name: 'learner-community-facilitator-summary',
	code: function (param, response)
	{	
		var id = app.get(
		{
			scope: 'learner-community-facilitator-summary',
			context: 'id',
			valueDefault: ''
		});
		...
		var data = app.find(
		{
			dataController: 'learner-community-facilitators',
			dataContext: 'all',
			controller: 'learner-community-facilitator-summary',
			context: 'id'
		});
		...
		{
			scope: 'learner-community-facilitator-summary',
			selector: '#learner-community-facilitator-summary',
			data: data
		});
	}	
});

...
app.add(
{
	name: 'learner-task-edit',
	code: function (param, response)
	{	
		var id = app._util.param.get(param, 'id', {default: ''}).value;
		...
		else
		{
			...
			{
				scope: 'learner-task-edit',
				selector: '#learner-task-edit',
				data: data
			});

			app.invoke('util-view-select',
			{
				container: 'learner-task-edit-type-' + id,
				object: 'setup_project_task_type',
				fields: [{name: 'title'}]
			});
		}		
	...
	{	
		...
		{		
			mydigitalstructure.cloud.save(
			{
				object: 'project_task',
				data: data,
				callback: 'learner-task-edit-save',
				set: {scope: 'learner-task-edit', guid: true},
		...
		{	
			if (response.status == 'OK')
			{
				app.invoke('app-navigate-to', {controller: 'learner-task-summary', context: _.first(response.data.rows).guid});
			}
		}
	}
});

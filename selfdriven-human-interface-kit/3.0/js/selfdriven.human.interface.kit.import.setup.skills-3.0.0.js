		...
		var importSetupSkills =
		{
			...
			initialise:
			{
				...
				[
					...
					{
						object: 'setup_contact_attribute',
						name: 'contact-attribute-skills-parents',
						fields:
						[
							{name: 'title'},
						],
...
app.add(
{
	...
	{	
		...
		_.each(importSetupSkills, function (importSetupSkill, importSetupSkillIndex)
		{
			...
			{
				return _.toLower(category.title) == _.toLower(importSetupSkill.categoryName)
			});

			if (_.isSet(importSetupSkill._category)) {importSetupSkill.categoryCode = importSetupSkill._category.reference}

			// TYPE (DOMAIN)
			importSetupSkill._type = _.find(inStorage['contact-attribute-skills-type'], function (type)
			{
				return _.toLower(type.title) == _.toLower(importSetupSkill.typeName)
			});

			if (_.isSet(importSetupSkill._type)) {importSetupSkill.typeCode = importSetupSkill._type.reference}

			// LEVEL
			importSetupSkill._level = _.find(inStorage['contact-attribute-skills-level'], function (level)
			{
				return _.toLower(level.title) == _.toLower(importSetupSkill.levelName)
			});

			if (_.isSet(importSetupSkill._level)) {importSetupSkill.levelCode = importSetupSkill._level.reference}

			// CAPACITY
			importSetupSkill._capacity = _.find(inStorage['contact-attribute-skills-capacity'], function (capacity)
			{
				return _.toLower(capacity.title) == _.toLower(importSetupSkill.capacityName)
			});

			if (_.isSet(importSetupSkill._capacity)) {importSetupSkill.capacityCode = importSetupSkill._capacity.reference}

			if (_.isNotSet(importSetupSkill['skillURI']))
			{
				importSetupSkill['skillURI'] = 
					importSetupSkill['categoryCode'] +
					importSetupSkill['typeCode'] +
					importSetupSkill['skillCode'] +
			...
			{
				importSetupSkill['skillName'] = importSetupSkill['skillName'] + ' [' + _.last(importSetupSkill['levelCode']) + '-' + importSetupSkill['capacityName'] + ']'
			}
				
			importSetupSkill.validate = {};

			if (_.isNotSet(importSetupSkill['skillName']))
			{	
				errors.push('Skill Name is required.');
				importSetupSkill.validate.skillName = false;
			}

			if (_.isNotSet(importSetupSkill['skillCode']))
			{	
				errors.push('Skill Code is required.');
				importSetupSkill.validate.skillCode = false;
			}

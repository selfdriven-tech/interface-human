...
[
	{
		...
		code: function ()
		{
			...
			console.log('https://selfdriven.community');
			console.log('https://selfdriven.foundation/intro-cardano');
	...
	},
	{
		...
		code: function ()
		{
			...
			else
			{
				...
				[
					'<div class="mt-4">',
						'<div class="font-weight-bold text-secondary lead">',
							'Please connect your Cardano wallet to continue.',
						'</div>',
						'<div class="mt-2 text-secondary">Thank you.</div>',
					'</div>'
				]);
				...
				if (_.has(community, 'mySetup.email'))
				{
					communityView.add(
					[

						'<div class="mt-4">',
							'<div class="text-secondary">If you need help please contact us @</div>',
							'<div><a class="text-info" href="mailto:', community.mySetup.email, '">', community.mySetup.email, '</a></div>',
						'</div>'
					]);
				}
	...
	},
	{
		...
		code: function (param)
		{
			...
			$('#community-view').html('<div class="mt-4 text-secondary">Connecting...</div>');
    ...
    {
		name: 'community-wallet-disconnect',
		code: function (param)
		{
            location.reload()
        }
	...
	},
	{
		...
		code: function (param)
		{
			...
			// SET THIS IN THE community specific js
			/*const communityAuth = eos.set(
			{
			...
			var communityAuth = eos.get(
			{
				scope: 'community-wallet-auth-using-policy',
				context: 'auth'
			});

			communityAuth.assetBasedOnPolicyIDFound = false;

			// Is this in Sign Up Mode

			//check data.signup

			var _communityWalletAssetsView = eos.view();
			...
			if (data.signup)
			{
				...
				if (_.isSet(data['signup-context']))
				{
					//Assume event for now
					if (_.has(data, 'mySetup.events'))
					{
						const event = _.find(data.mySetup.events, function (event)
						{
							return event.reference == data['signup-context']
						});
						...
						if (_.isSet(event))
						{
							...
							[
								'<div class="mt-3 pb-1">',
									...
									'<div class="text-muted">', event.reference, '</div>',
								'</div>'
							]);
						}
					}
					...
					'<div class="mt-3 pb-1">',
						'<form role="form" class="mx-auto text-center">',
							'<div class="form-group form-floating">',
								'<input type="text" class="form-control entityos-text text-center input-rounded" data-scope="community-sign-up" data-context="memberfirstname" placeholder="First Name">',
							'</div>',
							'<div class="form-group form-floating">',
								'<input type="text" class="form-control entityos-text text-center input-rounded" data-scope="community-sign-up" data-context="memberlastname" placeholder="Last Name">',
							'</div>',
							'<div class="form-group form-floating">',
								'<input type="text" class="form-control entityos-text text-center input-rounded" data-scope="community-sign-up" data-context="memberemail" placeholder="Email">',
							'</div>',
						'</form>',
					'</div>'
				]);
					...
					'<div class="mt-3 pb-1">',
						'<div class="mt-3" id="community-wallet-auth-view">',
							' <button type="button"',
									...
									' data-controller="community-auth-sign-up"',
									' data-policy="', communityAuth.policy, '"',
								'>',
								'Sign Up',
							'</button>',
						'</div>',
					'</div>',
					'<div id="community-auth-sign-up-message-view"></div>'
				]);
			}
			else
			{
				if (_wallet.canSignData)
				{
					...
					{
						if (_otherAsset.policy == communityAuth.policy)
						{
							communityAuth.assetBasedOnPolicyIDFound = true;
			
							_communityWalletAssetsView.add(
							[
								'<div>',
									'<h4 class="my-2">',
					...
					if (communityAuth.assetBasedOnPolicyIDFound)
					{
						communityWalletAssetsView.add(
						[
							'<div class="mt-3 pb-1">',
								_communityWalletAssetsView.get(),
								'<div class="text-muted small">',
									'Policy # <a class="text-muted" href="https://cardanoscan.io/tokenPolicy/',
										communityAuth.policy, '" target="_blank">', communityAuth.policy, '</a>',
								'</div>',
								'<div class="mt-3" id="community-wallet-auth-view">',
									' <button type="button"',
											' id="community-wallet-auth-using-policy"',
						...
						'<div class="mt-3 pb-1">',
							...
							'<div class="mt-3" id="community-wallet-auth-view">',
								'<div class="text-secondary">',
									'<div><i class="fa lead fa-info-circle"></i></div><div class="mt-2 font-weight-bold">Currently you can not use a Hardware Wallet to sign data on Cardano.</div><div class="mt-1">Hardware Wallet support is being worked on by the Cardano Community, and when available they will work automatically with this site.</div>',
								'</div>',
							'</div>',
						'</div>'
					]);
	...
	},
	{
		...
		code: function (param)
		{
            ...
            {   
                eos.invoke('community-wallet-auth-using-policy-logon-init')
            }
        }
    },
	{
		name: 'community-wallet-auth-using-policy-logon-init',
		code: function (param, response)
		{
			if (response == undefined)
			{
				entityos.cloud.invoke(
                {
                    ...
                    data: {},
                    callback: 'community-wallet-auth-using-policy-logon-init'
                })
			}
			else
			{
				...
				{
					scope: 'community-wallet-auth-using-policy-logon',
					context: 'logonkey',
					value: response.logonkey
				});

				const communityAuth = eos.get(
				{
					scope: 'community-wallet-auth-using-policy',
					context: 'auth'
				});
				...
				var dataToSign = communityAuth.policy;
					...
					function (addresses)
					{
						...
						{						
							eos.set(
							{
							 ...
							 eos.invoke('community-wallet-auth-using-policy-logon')
						},
						function(error)
						{
							var authOnChainView = eos.view();

							let errorMessage = error.info;
							...
							if (errorMessage == undefined)
							{
								errorMessage = 'An error has occured when signing the data.';
							}

							authOnChainView.add(
							[
								'<div class="pt-4 text-white fw-bold">', errorMessage, '</div>'
							])
    ...
    {
		name: 'community-wallet-auth-using-policy-logon',
		code: function (param, response)
		{
			//var policy = _.get(param.dataContext, 'policy');
            ...
            if (response == undefined)
            {
                ...
                {      
                    ...
                    apikey: communityAuth.apikey,
                    data:
                    {
                        site: communityAuth.site,
                        auth:
                        {
                            key: dataLogon.signeddata.key,
                            signature: dataLogon.signeddata.signature,
                            datatoverify: communityAuth.policy + '|[[LOGONKEY]]',
                ...
                entityos.cloud.invoke(
                {
                    ...
                    callback: 'community-wallet-auth-using-policy-logon'
                });
            }
            else
            {
				...
				else
				{
					...
					communityWalletAssetsView.render('#community-dashboard-view');
				}
            }		
		}
	},
...
eos.add(
{
	name: 'community-dashboard-learning-projects-clone-copy-save',
	code: function (param, response)
	{			
		...
		else
		{
			if (_.isUndefined(response))
			{
				...
				mydigitalstructure.cloud.save(
				{
					object: 'project',
					data: data,
					callback: 'community-dashboard-learning-projects-clone-copy-save'
				});
			}
			else
			{	
				if (response.status == 'OK')
				{
					eos.set( 
					{
						scope: 'community-dashboard-learning-project-do',
						value: _.first(response.data.rows)
					});

					eos.invoke('community-dashboard-learning-projects-clone-copy-team-init')
				}
			}
		}
	}
...
eos.add(
{
	name: 'community-dashboard-learning-projects-clone-copy-team-init',
	code: function (param, response)
	{	
		if (response == undefined)
		{
			entityos.cloud.search(
			{
				...
				callback: 'community-dashboard-learning-projects-clone-copy-team-init'
			});
		}
		else
		{
			eos.set( 
			{
				scope: 'community-dashboard-learning-projects-clone-copy-team',
				value: response.data.rows
			});

			eos.set( 
			{
				scope: 'community-dashboard-learning-projects-clone-copy-team-index',
				value: 0
			});

			eos.invoke('community-dashboard-learning-projects-clone-copy-team-process')
		}
	}
});
...
eos.add(
{
	name: 'community-dashboard-learning-projects-clone-copy-team-process',
	code: function (param, response)
	{	
		...
		if (projectCloneTeamConnectionIndex < projectCloneTeamConnections.length)
		{
			...
			entityos.cloud.save(
			{
				object: 'project_team',
				data: data,
				callback: 'community-dashboard-learning-projects-clone-copy-team-next',
				callbackParam: param
			});
		}
		else
		{
			app.invoke('community-dashboard-learning-projects-clone-copy-complete');
		}
	}
});
...
eos.add(
{
	name: 'community-dashboard-learning-projects-clone-copy-team-next',
	code: function (param, response)
	{	
		const projectCloneTeamConnectionIndex = eos.get( 
		{
		...
		app.invoke('community-dashboard-learning-projects-clone-copy-team-process');
	}
});

eos.add(
{
	name: 'community-dashboard-learning-projects-clone-copy-complete',
	code: function (param, response)
	{
		eos.invoke('community-dashboard-learning-project-do')
	}
});		

eos.add(
{
	name: 'community-dashboard-learning-project-do',
	code: function (param, response)
	{
		...
		const communityDashboardView = eos.view();
		...
		[
			'<div class="row mt-2 mb-4">',
				'<div class="col-12 mt-3 mt-md-3">',
					'<div class="d-none">',
						'<a class="btn btn-primary rounded-pill entityos-click" data-controller="community-dashboard-show"><i class="fas fa-home text-white"></i></a>',
					'</div>',
					'<div id="project-do-status-view"></div>',
		...
		eos.invoke('util-project-do', {id: projectID, viewSelector: '#community-dashboard-learning-project-do-view'})
	}
});

eos.add(
{
	name: 'community-auth-sign-up',
	code: function (param, response)
	{
		const data = entityos._util.data.get(
		{
			scope: 'community-sign-up',
			valueDefault: {}
		});

		const community = entityos._util.data.get(
		{
			scope: 'community',
			valueDefault: {}
		});
		...
		if (response == undefined)
		{
			...
			community.member =
			{
				identifiers:
				{
					onchain:
					{
						address: _.first(community._wallet.addresses.used).address,
						account: _.first(community._wallet.addresses.reward).address,
					},
					username: data.email
				}
			}

			if (_.isNotSet(community.member.identifiers.username) && _.has(community.member.identifiers, 'onchain.account'))
			{
				community.member.identifiers.username = blake2bHex(community.member.identifiers.onchain.account, null, 24)
			}
			...
			let invokeData =
			{
				...
				{
					_context: _context,
					source: 'app.dgov.education',
					identifiers: JSON.stringify(community.member.identifiers),
			...
			if (_.isNotSet(community['auth-settings']))
			{
				community['auth-settings'] = {}
			}

			if (_.isNotSet(community['auth-settings'].usernamepath))
			{
				community['auth-settings'].usernamepath = 'member.identifiers.username'
			}

			invokeData.data.memberusername = _.get(community, community['auth-settings'].usernamepath);
			...
			/*entityos.cloud.invoke(
			{
				...
				type: 'POST',
				callback: 'community-auth-sign-up',
				callbackParam: param
			});*/
		}
		else

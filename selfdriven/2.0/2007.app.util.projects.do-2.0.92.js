		...
		_.each(template.milestones, function (milestone)
		{
			...
			if (milestone._canBeEdited)
			{
				milestone.header =
					'<a class="entityos-click" data-controller="util-project-do-milestone-init"' +
						' data-reference="' + milestone.reference + '">' +
						'<h2 class="text-primary mb-2 badge border border-primary">' + milestone.reference + '</h2>' +
	...
	code: function (param)
	{
		...
		[
			'<div class="row align-items-end mt-4">',
				'<div class="col">',
					'<h1 class="mb-1 text-primary" style="font-size: 1.6rem;">',
						milestone.headerCaption,
					'</h1>',
				'</div>',
		...
		_.each(milestone.tasks, function (task, taskIndex)
		{
			...
			if (_.isSet(task.reference))
			{
				task.headerSubject = 
					task['subject'] +
						'<span class="badge border-primary border rounded text-primary ml-2" style="font-size:0.8rem;font-weight: 100;">' +
							task.reference +
						'</span>';
			}
			else
			...
			{
				task.headerMore =
					_.join(['<a class="btn btn-sm btn-white border-primary shadow myds-collapse-toggle" data-toggle="collapse" role="button"',
						' href="#util-project-do-milestone-tasks-', taskIndex, '-collapse"',
						' id="util-project-do-milestone-tasks-', taskIndex, '-collapse-container"',
						' data-reference="', milestone.reference, '">',
						'<i class="fa fa-chevron-up text-primary mt-0"></i>',
					'</a>'], '')
			}
			else
			{
				task.headerMore =
					_.join(['<a class="btn btn-sm btn-white border-primary shadow myds-collapse-toggle" data-toggle="collapse" role="button"',
						...
						' data-reference="', milestone.reference, '"',
						'>',
						'<i class="fa fa-chevron-down text-primary mt-0"></i>',
					'</a>'], '')
			}
			...
			if (template.type == 'project')
			{
				if (projectTaskInProgress)
				{
					_actionsHTML.add(['<div class="mt-4"',
							...
							'<button type="button" class="btn btn-default btn-outline btn-outline-primary btn-sm entityos-click"',
							...
							'</button>',
							(!percentComplete100?
							'<button type="button" class="btn btn-default btn-outline btn-outline-primary btn-sm entityos-click ms-2"' +
								' data-controller="util-project-do-milestone-task-save"' +
								' data-milestone="' + milestone.reference + '"' +
								' data-hash="' + task.hash + '"' +
								' data-task-index="' + taskIndex + '"' +
				...
				if (projectTaskInProgress)
				{
					_actionsHTML.add(
						[
							'<div class="mt-4"',
								' id="util-project-do-milestone-task-', task.hash, '">',
								'<button type="button" class="btn btn-sm btn-default btn-outline btn-outline-primary entityos-click"',

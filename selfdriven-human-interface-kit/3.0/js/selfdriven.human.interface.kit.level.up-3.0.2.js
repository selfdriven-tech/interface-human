...
var app =
{
	build:
	{
		name: 'selfdriven Level Up',
		version: '3.0.1',
		description: "selfdriven Level Up app",
		author: {
...
app.options = 
{
	...
	startURI: '/app-level-up',
	startURIContext: '#level-up-dashboard',
	start:
	[
		{
			urlContains: 'levelup',
		...
		[
			{
				...
				roles: ['learner', 'learning-partner']
			},
			{
				uri: '/app-admin',
				uriContext:
			...
			},
			{
				...
				uriContext:
				[
					'#util-identity-dashboard',
					'#util-identity-account'
				],
				onlyApplyIfURIDataContextSet: false,
				applyEvenIfReload: true
			},
...
app.add(
{
	...
	code: function (param)
	{
		...
		if (false && _.has(entityos, '_scope.user.roles.rows'))
		{
		...
		//if (param != undefined)
		//{
			_.set(param, 'uri', app.options.startURI);
			_.set(param, 'uriContext', app.options.startURIContext);
		//}	
	}
});

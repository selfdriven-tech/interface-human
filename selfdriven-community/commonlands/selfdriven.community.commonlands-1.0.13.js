	...
	{	
		...
		if (response == undefined)
		{
			...
			if (
				...
				|| 	_.isNotSet(communitySignupData.mobile)
				|| 	_.isNotSet(communitySignupData.firstname)
				|| 	_.isNotSet(communitySignupData.userpassword)
				|| 	_.isNotSet(communitySignupData.userpasswordverify)			
			)
			{
				messageView.add('<i class="fa fa-exclamation-triangle text-warning mr-2"></i>You are missing some mandatory data');
			}
			else
			{
				if (communitySignupData.userpassword !=
						communitySignupData.userpasswordverify)
				{
					messageView.add('<i class="fa fa-exclamation-triangle text-warning mr-2"></i>Passwords do not match');
				}
				else
				{
					const checkText = eos.invoke('util-check-text',
					{
						text: communitySignupData.userpassword,
						minimumLength: 8
					});
					...
					if (!checkText.ok)
					{
						messageView.add(checkText.error)
					}
					else
					{
			...
			if (validationOK)
			{
				let _context = 'selfdriven-community-2ab88512-3d01-47ac-8616-d5898d62161b-commonlands-prod';
				let communitykey = '2ab88512-3d01-47ac-8616-d5898d62161b';
				
				if (app.whoami().mySetup.isLab)
				{
					_context = 'selfdriven-community-7a4fbf7a-5ff4-416c-a864-af3604719dab-commonlands-lab';
					communitykey = '7a4fbf7a-5ff4-416c-a864-af3604719dab'
				}
		...
		else
		{
			...
			else
			{
				...
				let message = 'There was an issue signing you up, please contact Commonlands support @';
				if (response.errorCode == '5EMM3E')
				{
					message = 'A user with this name and mobile number already exists, please contact Commonlands support @';
				}
				
				messageView.add(
				[
					'<h4 class="mt-3 pb-3 text-white"><div><i class="fa fa-exclamation-triangle text-warning mr-2"></i>', message, '',
					'</div><div class="mt-1 text-white"><a href="mailto:connect@commonlands.org" class="text-white">connect@commonlands.org</a></div></h4>'
				]);
...
eos.add(
{
	name: 'community-dashboard-check',
	code: function (param, response)
	{	
		var communityData = eos.get({scope: 'community'});

		if (communityData.signup)
		{
			$('#community-signup-view').removeClass('d-none');
		}
		else
		{
			if (response == undefined)
			{
				...
				{
					method: 'core_get_user_details',
					callback: 'community-dashboard-check',
					callbackParam: param
				});
			}
			else
			{
				...
				{
					$('#community-auth-view').removeClass('d-none');
				}
				else
				{
					entityos._scope.user = response;
					entityos._scope.user.id = entityos._scope.user.user;
					eos.invoke('community-dashboard-show')
				}
			}
		}
	}
...
function communityInit()
{
	...
	entityos._scope.app.options.deauthURI = 'https://learn.commonlands.org';
	//entityos._scope.app.options.deauthURI = 'https://community-lab.selfdriven.cloud/commonlands';
	//app.invoke('util-cloud-check', {onComplete: 'community-dashboard-show'})
	eos.invoke('community-dashboard-check');
}
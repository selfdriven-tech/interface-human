						...
						'<div class="col-12 text-center mb-3">',
							'<div class="card shadow">',
								'<div class="card-body ">',
									...
									'<a class="btn btn-outline-primary btn-sm rounded-pill entityos-click"',
		...
		if (_.get(param, 'status') == 'shown')
		{
			...
			else
			{
				...
				if (response.data.rows.length == 0)
				{
					...
					[
						'<div class="col-12 text-center mb-3">',
							'<div class="font-weight-bold">Sorry, there are no learning templates available.</div>',
						'</div>'
					]);
				}
				else
...
eos.add(
{
	...
	code: function (param, response)
	{
		...
		eos.invoke('util-project-do', {id: project.id, viewSelector: '#community-dashboard-learning-project-do-view'})
	}
});

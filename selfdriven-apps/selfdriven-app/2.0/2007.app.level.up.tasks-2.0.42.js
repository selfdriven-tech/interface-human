	...
	code: function (param, response)
	{
		...
		if (response == undefined && taskTypes == undefined)
		{
			mydigitalstructure.cloud.search(
			{
				...
				callback: 'level-up-tasks'
			});
		}
		else
		{
			var typesView = app.vq.init({queue: 'level-up-tasks-types'});
			typesView.template(
			[
				'<li>',
			...
			typesView.add(
			[
				'<button type="button" class="btn btn-sm btn-primary dropdown-toggle" data-toggle="dropdown" id="level-up-tasks-types-filter" aria-expanded="false">',
					'<span class="dropdown-text">Type</span>',
				'</button>',
				'<ul class="dropdown-menu mt-1"',
					'data-controller="level-up-tasks-dashboard"',
					'data-context="type"',
				'>',
				'<li>',
			...
			{
				typesView.add({useTemplate: true, queue: 'level-up-tasks-types'}, type)
			});

			typesView.add('</ul>');

			typesView.render('#level-up-tasks-dashboard-types');

			app.invoke('level-up-tasks-dashboard');
		}
	}
});
...
app.add(
{
	name: 'level-up-tasks-dashboard',
	code: function (param, response)
	{
		var data = app.get(
		{
			scope: 'level-up-tasks-dashboard',
			valueDefault: {}
		});
		...
		{
			object: 'project_task',
			container: 'level-up-tasks-dashboard-view',
			context: 'level-up-tasks',
			filters: filters,
			options:
			{
			...
			format:
			{
				...
				row:
				{
					data: 'data-id="{{id}}"',
					class: 'd-flex',
					controller: 'level-up-tasks-dashboard-format'
				},
				...
				[
					{
						...
						data: 'data-context="{{guid}}" data-id="{{guid}}" data-controller="level-up-task-summary"'
					},
					{
						caption: 'Status',
						field: 'statustext', 	
...
app.add(
{
	name: 'level-up-tasks-dashboard-format',
	code: function (row)
	{
...
app.add(
{
	name: 'level-up-task-summary',
	code: function (param, response)
	{	
		var guid = app.get(
		{
			scope: 'level-up-task-summary',
			context: 'id'
		});

		if (guid == undefined || guid == '')
		{
			app.invoke('app-navigate-to', {controller: 'level-up-tasks'});
		}
		else
		{
			if (response == undefined)
			{
				mydigitalstructure.cloud.search(
				{
					...
					set: 
					{
						scope: 'level-up-community-tasks',
						context: 'all'
					},
					callback: 'level-up-task-summary'
				});
			}
			else
			{
				if (response.status == 'OK')
				{
					...
					else
					{
						...
						{
							scope: 'level-up-task-summary',
							selector: '#level-up-task-summary',
							data: data,
							collapse: {contexts: []}
						});
						...
						{
							container: 'level-up-task-summary-status',
						...
						app.invoke('util-actions-show',
						{
							dataContext:
							{
								container: 'level-up-task-summary-actions-view',
								context: 'level-up-task-summary',
								object: app.whoami().mySetup.objects.projectTask,
								objectcontext: data.id
							},
						...
						app.invoke('level-up-task-summary-skill-capacity-show');
					}
				}
			}
		}
...
app.add(
{
	name: 'level-up-task-edit-status-save',
	code: function (param, response)
	{	
		var taskID = app.get(
		{
		...
		{
			scope: 'level-up-task-edit-status-' + taskID,
		...
		{		
			mydigitalstructure.cloud.save(
			{
				object: 'project_task',
				data: data,
				callback: 'level-up-task-edit-status-save',
			});
		}
		else
		{	
			if (response.status == 'OK')
			{
				app.refresh(
				{
					dataScope: 'level-up-tasks',
					data: data
				});

				app.invoke('level-up-task-edit-status-comment-save');
			}
		}
	}
});
...
app.add(
{
	name: 'level-up-task-edit-status-comment-save',
	code: function (param, response)
	{	
		var taskID = app.get(
		{
		...
		{
			app.invoke('level-up-task-edit-status-save-complete');
		}
		else
		{
			var utilSetup = app.get(
			...
			{		
				mydigitalstructure.cloud.save(
				{
					object: 'action',
					data: data,
					callback: 'level-up-task-edit-status-comment-save',
				});
			}
			else
			{	
...
app.add(
{
	name: 'level-up-task-edit-status-save-complete',
	code: function (param, response)
	{	
		...
		app.invoke('level-up-task-summary');
		//app.invoke('app-navigate-to', {scope: 'level-up-task-summary', context: taskID});
	}
});

app.add(
{
	name: 'level-up-task-summary-reflection-save',
	code: function (param, response)
	{	
		var taskID = app.get(
		{
		...
		{
			scope: 'level-up-task-summary-reflection-' + taskID,
			cleanForCloudStorage: true,
			mergeDefault:
			{
				values:
		...
		{		
			mydigitalstructure.cloud.save(
			{
				...
				callback: 'level-up-task-summary-reflection-save',
			});
		}
		else
		{	
...
app.add(
{
	name: 'level-up-task-summary-skill-capacity-show',
	code: function (param, response)
	{
		var task = app.get(
		{
		...
		if (response == undefined)
		{
			mydigitalstructure.cloud.search(
			{
				...
				rows: 1,
				callback: 'level-up-task-summary-skill-capacity-show',
				callbackParam: param
			});
		}
		else
		{
			...
			{
				$('#level-up-task-summary-skill-capacity-' + taskSkillCapacity.objectcontext + '-' + task.id).attr('checked', 'checked');
			}
		}
	}
});
...
app.add(
{
	name: 'level-up-task-summary-skill-capacity-save',
	code: function (param, response)
	{
		var task = app.get(
		{
		...
		{
			scope: 'level-up-task-summary-skill-capacity-' + task.id
		});
		...
		else
		{
			if (response == undefined)
			{
				entityos.cloud.save(
				{
					...
					callback: 'level-up-task-summary-skill-capacity-save'
				});
			}
			else
			{

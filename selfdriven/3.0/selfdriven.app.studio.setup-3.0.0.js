...
app.add(
{
	...
	code: function (param, response)
	{
		...
		{
			object: 'project',
			container: 'studio-setup-templates-view',
			context: 'studio-setup-templates',
			filters: filters,
			options:
			{
			...
			format:
			{
				...
				[
					...
					},
					{
						...
						field: 'reference', 	
						sortBy: true,
						class: 'col-5 myds-navigate text-muted',
						data: 'data-context="{{guid}}" data-controller="studio-setup-template-summary"'
					},
					{	
						fields:
						['guid', 'type', 'typetext', 'subtype', 'subtypetext', 'contactbusinesstext', 'contactpersontext', 'status']
					}
				]
			}
		});
...
app.add(
{
	name: 'studio-setup-template-summary',
	code: function (param, response)
	{	
		var guid = app.get(
		{
			scope: 'studio-setup-template-summary',
			context: 'context'
		});

		const templateSaveRequired = app.get(
		{
		...
		if (templateSaveRequired)
		{
			app.invoke('util-template-setup-edit-save')
		}

		if (guid == undefined || guid == '')
		{
			app.invoke('app-navigate-to', {controller: 'studio-setup-templates'});
		}
		else
		{
			if (response == undefined)
			{
				mydigitalstructure.cloud.search(
				{
					...
					callback: 'studio-setup-template-summary'
				});
			}
			else
			{
				if (response.status == 'OK')
				{
					...
					else
					{
						...
						data._publishButton = '<button class="btn btn-default btn-outline btn-sm myds-click"' +
								' data-controller="studio-setup-template-status-save"' +
								' data-type="publish"' +
								' data-context="' + data.id + '" data-id="' + data.id + '">' +
									'Publish' +
								'</button>';

						if (data.status == app.whoami().mySetup.projectStatuses.completed)
						{
							data._publishButton = '<button class="btn btn-default btn-outline btn-sm myds-click"' +
								' data-controller="studio-setup-template-status-save"' +
								' data-type="unpublish"' +
						...
						{
							scope: 'studio-setup-template-summary',
							selector: '#studio-setup-template-summary',
							data: data,
							collapse:
							{
						...
						{
							context: 'studio-setup-template-summary',
...
app.add(
{
	name: 'studio-setup-template-edit',
	code: function (param, response)
	{	
		var data = app.get(
		{
		...
		app.refresh(
		{
			scope: 'studio-setup-template-edit',
			selector: '#studio-setup-template-edit',
			data: data
		});

		app.invoke('util-view-editor',
		{
			selector: '#studio-setup-template-edit-notes-' + data.id,
			height: '400px',
			content: data.notes
		});
		...
		{
			container: 'studio-setup-template-edit-type-' + data.id,
			...
			filters: [
			{
				...
				comparison: 'IN_LIST',
				value: [
					app.whoami().mySetup.projectTypes.learning,
					app.whoami().mySetup.projectTypes.verification,
					app.whoami().mySetup.projectTypes.facilitation,
					app.whoami().mySetup.projectTypes.management
				]
			}]
		});
		...
		app.invoke('util-view-select',
		{
			container: 'studio-setup-template-edit-category-' + data.id,
...
app.add(
{
	name: 'studio-setup-template-edit-save',
	code: function (param, response)
	{	
		var id = app.get(
		{
		...
		var data = app.get(
		{
			scope: 'studio-setup-template-edit-' + id,
			...
			mergeDefault:
			{
				...
				values:
				{
					...
					category: app.whoami().mySetup.projectTypes.notSet
				}
			}
		});
		...
		{		
			mydigitalstructure.cloud.save(
			{
				object: 'project',
				data: data,
				callback: 'studio-setup-template-edit-save',
				set: {scope: 'studio-setup-template-edit-edit', data: true, guid: true},
		...
		{	
			if (response.status == 'OK')
			{
				...
				{
					app.invoke('studio-setup-template-edit-save-reference', param, response)
				}
				else
				{
					app.invoke('studio-setup-template-edit-save-finalise')
				}
			}
		}
	}
...
app.add(
{
	name: 'studio-setup-template-edit-save-reference',
	code: function (param, response)
	{	
		if (response != undefined)
		{
			...
			mydigitalstructure.cloud.save(
			{
				object: 'project',
				data: data,
				callback: 'studio-setup-template-edit-save-finalise',
				set: {scope: 'studio-setup-template-edit'}
			});
		}
	}
});
...
app.add(
{
	name: 'studio-setup-template-edit-save-finalise',
	code: function (param, response)
	{	
		var id = app.get(
		{
		...
		{
			dataScope: 'studio-setup-templates',
			data: data
		});

		app.invoke('app-navigate-to', {controller: 'studio-setup-template-summary', context: data.guid});
	}
});

app.add(
{
	name: 'studio-setup-template-summary-task-edit',
	code: function (param)
	{	
		var projectID = app.get({scope: 'studio-setup-template-summary-task-edit', context: 'dataContext'});
		...
		app.invoke('util-view-select',
		{
			container: 'studio-setup-template-summary-task-edit-type',
...
app.add(
{
	name: 'studio-setup-template-summary-task-edit-save',
	code: function (param, response)
	{	
		var projectID = app.get(
		{
			scope: 'studio-setup-template-summary-task-edit',
			context: 'dataContext',
			name: 'project'
		});
		...
		*/
		...
		var data = app.get(
		{
			controller: 'studio-setup-template-summary-task-edit-' + id,
			cleanForCloudStorage: true,
			valueDefault: {}
		});
		...
		if (_.isUndefined(response))
		{
			mydigitalstructure.update(
			{
				object: 'project_task',
				data: data,
				callback: 'studio-setup-template-summary-task-edit-save',
				callbackParam: param
			});
		}
		else
		{	
			if (response.status == 'OK')
			{
				...
				app.invoke('util-view-collapse', {context: 'studio-setup-template-summary-task-edit'});
				app.invoke('studio-setup-template-summary-tasks-show', {status: 'shown'})
			}
		}
	}
});
...
app.add(
{
	name: 'studio-setup-template-summary-tasks-show',
	code: function (param, response)
	{	
		...
		var selector = app._util.param.get(param.dataContext, 'selector',
				{ default: 'studio-setup-template-summary-tasks-view'}).value;
		...
		if (scope == undefined)
		{
			scope = 'studio-setup-template-summary-tasks-show'
		}
		...
		if (param.status == 'shown')
		{
			if (projectID != undefined && selector != undefined)
			{
				...
				app.invoke('util-view-table',
				{
					object: 'project_task',
					container: selector,
					context: 'studio-setup-template-summary-tasks',
					filters: filters,
					options:
					{
						...
						deleteConfirm:
						{
							...
							buttonText: 'Delete',
							controller: 'studio-setup-template-summary-task-delete-ok'
						},
						showFooter: false
					},
					format:
						...
						[
							{
								...
								data: 'data-context="{{guid}}" data-id="{{guid}}" data-controller="studio-setup-template-task-summary"'
							},
							{
								caption: 'Info',
								name: 'info',
							...
							},
							{
								html: '<button class="btn btn-danger btn-outline btn-sm myds-delete"' +
									   ' id="studio-setup-template-summary-task-delete-{{id}}" data-id="{{id}}">' +
...
app.add(
{
	name: 'studio-setup-template-summary-task-delete-ok',
	code: function (param, response)
	{
		if (_.isUndefined(response))
		{
			if (!_.isUndefined(param.dataContext))
			{
				mydigitalstructure.cloud.delete(
				{
					...
					callback: 'studio-setup-template-summary-task-delete-ok'
				});
			}	
		}
		else
...
app.add(
{
	name: 'studio-setup-template-task-summary',
	code: function (param, response)
	{	
		var guid = app.get(
		{
			scope: 'studio-setup-template-task-summary',
			context: 'context'
		});
		...
		else
		{
			if (response == undefined)
			{
				mydigitalstructure.cloud.search(
				{
					...
					set: 
					{
						scope: 'studio-setup-template-tasks',
						context: 'all'
					},
					callback: 'studio-setup-template-task-summary'
				});
			}
			else
			{
				if (response.status == 'OK')
				{
					...
					else
					{
						...
						{
							scope: 'studio-setup-template-task-summary',
							selector: '#studio-setup-template-task-summary',
							data: data,
							collapse:
							{
						...
						app.invoke('util-attachments-initialise',
						{
							context: 'studio-setup-template-task-summary',
...
app.add(
{
	name: 'studio-setup-template-task-edit',
	code: function (param, response)
	{	
		var data = app.get(
		{
		...
		app.refresh(
		{
			scope: 'studio-setup-template-task-edit',
			selector: '#studio-setup-template-task-edit',
			data: data
		});

		app.invoke('util-view-select',
		{
			container: 'studio-setup-template-task-edit-type-' + data.id,
			object: 'setup_project_task_type',
			fields: [{name: 'title'}]
		});
		...
		app.invoke('util-view-select',
		{
			container: 'studio-setup-template-task-edit-milestone-' + data.id,
			hideSearch: true,
			data:
			[
				{
		...
		app.invoke('util-view-editor',
		{
			selector: '#studio-setup-template-task-edit-notes-' + data.id,
			height: '600px',
			content: data.notes
		});
	}	
...
app.add(
{
	name: 'studio-setup-template-task-edit-save',
	code: function (param, response)
	{	
		var id = app.get(
		{
		...
		{	
			var data = app.get(
			{
				scope: 'studio-setup-template-task-edit-' + id,
				cleanForCloudStorage: true,
				mergeDefault:
				{
					id: id,
			...
			mydigitalstructure.cloud.save(
			{
				object: 'project_task',
				data: data,
				callback: 'studio-setup-template-task-edit-save',
				set: {scope: 'studio-setup-template-task-edit', data: true, guid: true}
			});
		}
		else
		{	
			app.notify({message: 'Project template task has been ' + (id==''?'added':'updated') + '.'});
			app.invoke('app-navigate-to', {controller: 'studio-setup-template-task-summary', context: response.guid});
		}
	}
});
...
app.add(
{
	name: 'studio-setup-template-summary-projects-show',
	code: function (param, response)
	{	
		var projectID = app._util.param.get(param.dataContext, 'project').value;
		var scope = app._util.param.get(param.dataContext, 'scope').value;
				...
				app.invoke('util-view-table',
				{
					...
					options:
					{
						...
						deleteConfirm:
						{
							...
							buttonText: 'Remove',
							controller: 'studio-setup-template-summary-projects-delete-ok'
						},
						showFooter: false
					},
					format:
...
app.add(
{
	name: 'studio-setup-template-summary-information-show',
	code: function (param, response)
	{
		...
		if (response == undefined)
		{
			mydigitalstructure.cloud.search(
			{
				...
				rows: 1,
				callback: 'studio-setup-template-summary-information-show'
			});
		}
		else
		{
			...
			{
				containerSelector: '#studio-setup-template-summary-information-view'
			});
		}
	}
});
...
app.add(
{
	name: 'studio-setup-template-summary-information-edit',
	code: function (param)
	{
		...
		{
			param = app._util.param.set(param, 'template', template);
...
app.add(
{
	name: 'studio-setup-template-status-save',
	code: function (param, response)
	{	
		var dataContext = _.get(param, 'dataContext', {})
	
		let status = app.whoami().mySetup.projectStatuses.completed;

		if (dataContext.type == 'unpublish')
		{
			status = app.whoami().mySetup.projectStatuses.inProgress;
		}
		...
		{		
			entityos.cloud.save(
			{
				object: 'project',
				data: data,
				callback: 'studio-setup-template-status-save',
				callbackParam: param,
				notify: 'Template has been ' + dataContext.type + 'ed.'
			});
		}
		else
		{	
			if (response.status == 'OK')
			{
				const type = (dataContext.type=='publish'?'unpublish':'publish');
				const typeCaption = (dataContext.type=='publish'?'Unpublish':'Publish');

				$('[data-controller="studio-setup-template-status-save"]').data('type', type).html(typeCaption)
			}
		}
	}
});

	...
	code: function (param, response)
	{
		...
		if (response == undefined)
		{
			...
			else
			{
				...
				let systemMessages = ['You are a learning assistant.'];

				const lastMessage = _.first(messages);
				if (_.isSet(lastMessage))
				{
					systemMessages.push(lastMessage.ai);
				}
		...
		else
		{
			...
			else
			{
				...
				{
					user: userMessage,
					ai: _.get(response, 'data.messages.response', '').replace(/\n/g, '<br>')
				});
				...
				_.each(messages, function (message, m)
				{
					chatDashboardView.add(
					[
						'<div class="card mt-3">',

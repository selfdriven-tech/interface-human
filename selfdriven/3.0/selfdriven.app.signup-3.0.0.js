		...
		code: function (param) {
			...
			if (sourceType != 'keyup')
			{
				...
				if (dataRegister != undefined)
				{
					...
					$('#auth-register-learner-invite-view').html('<h2 class="text-center font-bold" style="color:#DE5A31 !important;">' + dataRegister.contactbusinesstext + '</h2>');
			...
			else
			{
				...
				if (response.status == 'ER')
				{
					if (response.error.errornotes == 'This email address is already used.' ||
						...
						response.errorName == 'community-member-user-account-exists')
					{
				...
				else
				{
					...
					if (_.has(data, 'sourcecontact'))
					{
						entityos._util.view.queue.add(
						[
							'<div class="text-secondary">' + data.sourcecontact  + '</div>'
						]);
					}
